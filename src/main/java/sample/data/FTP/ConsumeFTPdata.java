...
import static sample.data.models.BU.R.SchemaFields;
...
import sample.data.models.BU.R;
...
public class ConsumeFTPdata {
  private static final Logger LOG = LoggerFactory.getLogger(ConsumeFTPdata.class);
  ...
  public static PipelineResult run(KafkaStreamingOptions options)
    ...
    PCollection<R> predsTransformed = kafkaMessages
                .apply("Map to java class using class",
                        MapElements.into(TypeDescriptor.of(R.class))
                                .via(element -> {
                                    try {
                                        return element != null ? new R(element, env) : null;
